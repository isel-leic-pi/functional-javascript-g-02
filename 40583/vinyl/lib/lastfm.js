...
function searchArtist(artist, cb) {
    ...
    const url = LASTFM_HOST + LASTFM_SEARCH_ARTIST + artist + `&api_key=${LASTFM_KEY}&format=json`
    urllib.request(url,(err,data)=>{
        if(err) return cb(err)
        const obj = JSON.parse(data)
        // console.log(obj);
        if(obj.error) return cb(new Error(obj.message))
        const artistas = obj.results.artistmatches.artist;
        const artista = artistas.filter((a) => a.name.toUpperCase() == artist.toUpperCase())
        if(artista.length == 0){
